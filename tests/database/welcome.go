...
func withoutW(userId int) *bottypes.User {
	var (
		err  error
		user *bottypes.User
	)
...
func AfterGreetingsToUser(userId int) {
	user := withoutW(userId)
	if user.Level != 1 {
		panic("user.Level != 1")
	}
...
func AfterShowRules(userId int) {
	user := withoutW(userId)
	if user.Level != 2 {
		panic("user.Level != 2")
	}
...
func AfterWelcomeMainMenu(userId int) {
	user := withoutW(userId)
	if user.Level != 3 {
		panic("user.Level != 3")
	}
