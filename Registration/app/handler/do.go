...
func (b *btn) selGames(req *apptype.Request, con *Conn, f func(error), dict map[string]string) {
	...
	sch := con.selectTheSchedule(req.Limit, req.LaunchPoint, req.Language, f)
...
func makeSch(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn) {
	...
	b := new(btn)
	res.Level = 1
	b.selGames(req, con, fm.Error, dict.Dictionary[req.Language])
...
func presentationScheduele(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn) {
	producer.InterLogs("Start function Registration.presentationScheduele()",
		fmt.Sprintf("UserId: %d, req (*apptype.Request): %v, res (*apptype.Response): %v, fm (*formatter.Formatter): %v", req.Id, req, res, fm))
	res.Level = 3
	if con.findGame(fm.Error) {
		makeSch(req, res, fm, con)
	} else {
...
func makeDataForChSeats(res *apptype.Response, fm *formatter.Formatter, con *Conn, gameId int, dict map[string]string, sestat bool) {
	...
	res.Level = 2
	res.GameId = gameId
	price, space, currency := con.selectThePrice(res.GameId, fm.Error)
...
func chooseGame(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn, sstat bool) {
	...
	if det {
		if con.findThatGame(gid, fm.Error) {
			makeDataForChSeats(res, fm, con, gid, dict.Dictionary[req.Language], sstat)
		} else {
			makeSch(req, res, fm, con)
		}
	} else {
...
// Directs the main logic of this task
func chooseSeats(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn) {
	producer.InterLogs("Start function Registration.chooseSeats()",
		fmt.Sprintf("UserId: %d, req (*apptype.Request): %v, res (*apptype.Response): %v, fm (*formatter.Formatter): %v", req.Id, req, res, fm))
	det, seats := intCheck(req.Req)
	if det {
		if con.howManyIsLeft(req.GameId, seats, fm.Error) {
			makeDataForChPay(res, fm, seats, dict.Dictionary[req.Language])
		} else {
			makeDataForChSeats(res, fm, con, req.GameId, dict.Dictionary[req.Language], false)
		}
	} else {
		makeDataForChSeats(res, fm, con, req.GameId, dict.Dictionary[req.Language], true)
	}
	fm.WriteChatId(req.Id)
}

// There is a function which can make data to send (only if req.Req was "card")
func makeDataToPayByCard(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn, dict map[string]string) {
	producer.InterLogs("Start function Registration.makeDataToPayByCard()",
		fmt.Sprintf("UserId: %d, req (*apptype.Request): %v, res (*apptype.Response): %v, fm (*formatter.Formatter): %v", req.Id, req, res, fm))
	res.Level = 4
	price, _, currency := con.selectThePrice(req.GameId, fm.Error)
...
// Directs the main logic of this task
func choosePayment(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn) {
	producer.InterLogs("Start function Registration.choosePayment()",
		fmt.Sprintf("UserId: %d, req (*apptype.Request): %v, res (*apptype.Response): %v, fm (*formatter.Formatter): %v", req.Id, req, res, fm))
	if (req.Req == "card") || (req.Req == "cash") {
		if con.howManyIsLeft(req.GameId, req.Seats, fm.Error) {
			res.Payment = req.Req
			if req.Req == "card" {
				makeDataToPayByCard(req, res, fm, con, dict.Dictionary[req.Language])
			} else {
				bestWishes(req, res, fm, con)
			}
		} else {
...
// Makes body for wish
func wishUGoodLuck(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn, dict map[string]string) {
...
func bestWishes(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn) {
	producer.InterLogs("Start function Registration.bestWishes()",
		fmt.Sprintf("UserId: %d, req (*apptype.Request): %v, res (*apptype.Response): %v, fm (*formatter.Formatter): %v", req.Id, req, res, fm))
	if req.Req == "cash" || req.Req == "Next" {
		if con.howManyIsLeft(req.GameId, req.Seats, fm.Error) {
			ok := con.completeRegistration(req.Id, req.GameId, req.Seats, req.Payment, fm.Error)
			if ok {
				wishUGoodLuck(req, res, fm, con, dict.Dictionary[req.Language])
				upd(req, con, fm.Error)
			} else {
...
func dir(req *apptype.Request, res *apptype.Response, fm *formatter.Formatter, con *Conn) {
	...
	if req.Level == START {
		...
		presentationScheduele(req, res, fm, con)
	} else if req.Level == LEVEL1 {
		chooseGame(req, res, fm, con, true)
	} else if req.Level == LEVEL2 {
		chooseSeats(req, res, fm, con)
	} else if req.Level == LEVEL3 {
		choosePayment(req, res, fm, con)
